...
class CloudApi {
	...
	private $url = 'http://api.012wz.com/index.php?c=%s&a=%s&access_token=%s&';
	...
	private function developerCerContent(){
		...
		if (is_error($cer)) {
			return error(1, '访问云API获取授权失败,模块中没有开发者数字证书,请到 <a href="http://s.012wz.com/index.php?c=develop&a=auth" target="_blank">开发者中心</a> 下载数字证书!');;
		}
	...
	private function moduleCerContent(){
		...
		if (!is_file($cer_filepath)) {
			...
			$data = cloud_request('http://v2.addons.012wz.com/gateway.php', $pars);
			if (is_error($data)) {
				return $data;
			}
