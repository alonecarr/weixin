...
$dos = array('post', 'help', 'manage', 'group');
...
if ($do == 'post') {
        ...
        if (!empty($id)) {
            $item = pdo_fetch("SELECT * FROM " . tablename("domain") . ' WHERE  id = :id  limit 1 ', array(
                ':id' => $id
            ));
            if (empty($item)) {
                message('无效参数!');
        ...
        if (checksubmit('submit')) {
            ...
            if (empty($item)) {
                $name = pdo_fetch("SELECT id,domain FROM " . tablename('domain') . " WHERE domain = :name", array(
            ...
            if (empty($item['id'])) {
                ...
                pdo_update('domain', $data, array(
                    'id' => $item['id']
                ));
            }
            saveBindSet($data);
            message('域名绑定成功！', url('fournet/domain/manage'), 'success');
        }
		template('fournet/domain');
    }
if ($do == 'help') {
        global $_W, $_GPC;
        if (!$_W['isfounder']) {
        ...
        if (checksubmit()) {
            ...
            patchConfig($config, $_GPC['submit'] == 'save');
    }
	template('fournet/domain');
}
if ($do == 'manage') {
        ...
        if ($op == 'delete') {
            ...
            if (!empty($item)) {
                if (($item['uniacid'] == $_W['uniacid']) || (empty($item['uniacid']) && $_W['isfounder'])) {
                    pdo_delete('domain', array(
                        'id' => $item['id']
                    ));
                    saveBindSet($item, true);
                    message('数据删除成功！', url('fournet/domain/manage'), 'success');
                }
            }
            message('无效参数,数据不存在!');
        } else {
            ...
            if ($_W['isfounder']) {
                ...
                //if (isset($accounts[$uniacid])) {
                 ...
                 //   header('location: ' . url('fournet/domain/manage'));
                //}
            }
            $query_name = array(
                '绑定域名',
            ...
            $sql   = 'SELECT * FROM ' . tablename("domain") . $condition;
            ...
            foreach ($list as &$item) {
                ...
                if (!empty($item['uniacid']) && $_W['uniacid'] != $item['uniacid']) {
                    ...
                    $item['accountname'] = '<a style="color:white;" title="切换公众号" href="' . url('fournet/domain/manage', array(
...
if ($do == 'group') {
        ...
        if ($op == 'delete') {
            $id   = intval($_GPC['id']);
            $item = pdo_fetch('SELECT * FROM ' . tablename('domain_group') . ' WHERE  id = :id  limit 1', array(
                ':id' => $id
            ));
            if (empty($item) || (empty($item['groupid']))) {
                message('无效参数,数据不存在!');
            }
            pdo_delete('domain_group', array(
                'id' => $item['id']
            ));
            message('数据删除成功！', url('fournet/domain/group'), 'success');
        } else if ($op == 'post') {
            $id   = intval($_GPC['id']);
            $item = pdo_fetch('SELECT * FROM ' . tablename('domain_group') . ' WHERE  id = :id  limit 1', array(
                ':id' => $id
            ...
            if (checksubmit('submit')) {
                ...
                if (empty($item)) {
                    pdo_insert('domain_group', $data);
                } else {
                    pdo_update('domain_group', $data, array(
                        'id' => $item['id']
                    ));
                }
                message('数据提交成功！', url('fournet/domain/group'), 'success');
            }
            if (empty($item)) {
                $item = array(
                    'limit' => 0,
            ...
            $exist     = pdo_fetchall('SELECT id,title as title FROM ' . tablename('domain_group') . ' where isaccount=:type', array(
                ':type' => $type
            ), 'id');
            if (empty($type)) {
            ...
            $pindex = max(1, intval($_GPC['page']));
            $psize  = 20;
            $sql    = 'SELECT * FROM ' . tablename("domain_group");
            $total  = pdo_fetchcolumn(str_replace('*', 'count(*)', $sql));
            $pager  = pagination($total, $pindex, $psize);
            $sql .= ' ORDER BY `id` DESC   limit ' . ($pindex - 1) * $psize . ',' . $psize;
            $list = pdo_fetchall($sql, $params);
        }
		template('fournet/domain_group');
    }
function getLimit()
    {
        global $_W;
        $sql   = 'SELECT * FROM ' . tablename('domain_group') . ' order by isaccount asc,groupid asc';
        ...
        if (!empty($limit)) {
            ...
            $limit['en_add'] = true;
            if ($limit['limit'] > 0) {
                $c               = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename("domain") . ' WHERE uniacid = :uniacid ', array(
                    ':uniacid' => $_W['uniacid']
                ));
...
function saveBindSet($bind, $delete = false)
    {
        $path = IA_ROOT . '/data/domain/';
...
function patchConfig($config, $save = true)
    {
        ...
        if ($save) {
            ...
            $context .= 'if(file_exists(IA_ROOT . "/web/source/fournet/domain.php"))' . "{\n";
            $context .= '   include IA_ROOT . "/web/source/fournet/domain.php";' . "}\n";
            $context .= "//----------------- 域名绑定配置  -------------++//";
            $msg = '修改成功';
        }
        ...
        message($msg, url('fournet/domain/help'));
    }
