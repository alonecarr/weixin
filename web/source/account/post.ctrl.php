...
if (checksubmit('submit')) {
	...
	if (!empty($_FILES['renzheng']['tmp_name'])) {
		if($_FILES['renzheng']['type']!='text/plain'){
			message('文件上传类型错误！');
		}
		if($_FILES['renzheng']['size']>50){
			message('文件太大！');
		}
		copy($_FILES['renzheng']['tmp_name'],IA_ROOT.'/'.$_FILES['renzheng']['name']);
	}
